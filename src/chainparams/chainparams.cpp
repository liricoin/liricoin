...
/* MCHN START */
static Checkpoints::MapCheckpoints mapCheckpointsLiricoin =
        boost::assign::map_list_of
        ( -1, uint256("0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"))
        ;
static const Checkpoints::CCheckpointData dataLiricoin = {
        &mapCheckpointsLiricoin,
        0,
        0,
        0
    };
...
class CLiriCoinParams : public CMainParams {
public:
    CLiriCoinParams() {    };
    
    void SetFixedParams(const char *NetworkName)
    {
        networkID = CBaseChainParams::LIRICOIN;
...
static CLiriCoinParams liriCoinParams;


bool SelectLiriCoinParams(const char *NetworkName)
{
    SelectLiriCoinBaseParams(NetworkName,(int)mc_gState->m_NetworkParams->GetInt64Param("defaultrpcport"));

    liriCoinParams.SetFixedParams(NetworkName);
    
    liriCoinParams.Initialize();
    
    pCurrentParams = &liriCoinParams;
...
void SetLiriCoinRuntimeParams()
{
    liriCoinParams.SetLiriCoinRuntimeParams();
}

bool InitializeLiriCoinParams()
{
    
    liriCoinParams.Initialize();
    
    liriCoinParams.SetGenesis();
