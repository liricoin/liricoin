...
// LiriCoin code distributed under the GPLv3 license, see COPYING file.

#include "liricoin/liricoin.h"
...
int mc_OneLiricoinParam::IsRelevant(int version)
{
    int ret=1;
...
void mc_LiricoinParams::Zero()
{
    ...
    m_Size=0;
    m_IsProtocolLiriCoin=1;
    m_ProtocolVersion=0;
    
    m_AssetRefSize=MC_AST_SHORT_TXID_SIZE;
}

void mc_LiricoinParams::Destroy()
{
    if(m_lpData)
    {
        mc_Delete(m_lpData);
...
int64_t mc_LiricoinParams::GetInt64Param(const char *param)
{
    int size;
...
double mc_LiricoinParams::GetDoubleParam(const char *param)
{
...
void* mc_LiricoinParams::GetParam(const char *param,int* size)
{
    if(m_lpIndex == NULL)
    {
        return NULL;
...
int mc_LiricoinParams::SetParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc_LiricoinParams::SetParam(const char *param,int64_t value)
{
    int size;
    char buf[8];
    if(m_lpIndex == NULL)
...
void mc_LiricoinParams::Init()
{
    ...
    m_Count=sizeof(LiricoinParamArray)/sizeof(mc_OneLiricoinParam);
    max_size=0;
    
    for(i=0;i<m_Count;i++)
    {
        size=0;
        switch((LiricoinParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
        {
            case MC_PRM_BINARY  : size=(LiricoinParamArray+i)->m_MaxStringSize;   break;
            case MC_PRM_STRING  : size=(LiricoinParamArray+i)->m_MaxStringSize+1; break;
    ...
    m_lpParams=(mc_OneLiricoinParam*)mc_New(sizeof(LiricoinParamArray));
    m_lpData=(char*)mc_New(max_size);
    m_lpCoord=(int*)mc_New(2*m_Count*sizeof(int));
    
    memcpy(m_lpParams,LiricoinParamArray,sizeof(LiricoinParamArray));
    for(i=0;i<m_Count;i++)
    {
...
int mc_LiricoinParams::Create(const char* name,int version)
{
    int size,offset,i,set;
    mc_OneLiricoinParam *param;
    char *ptrData;
    int num_sets;
...
double mc_LiricoinParams::ParamAccuracy()
{
    return 1./(double)MC_PRM_DECIMAL_GRANULARITY;
}


int mc_LiricoinParams::Read(const char* name)
{
    return Read(name,0,NULL,0);
}

int mc_LiricoinParams::Read(const char* name,int argc, char* argv[],int create_version)
{
    ...
    mc_LiricoinParams *lpDefaultParams;
    ...
    lpDefaultParams = new mc_LiricoinParams;
...
/*
        if(argc == 0)
        {
            if((LiricoinParamArray+i)->m_Type & MC_PRM_SPECIAL)
            {
                if(strcmp((LiricoinParamArray+i)->m_Name,"rootstreamname") == 0)
                {
                    if(ptr == NULL)
                    {
...
*/
        ...
        {        
            ...
            switch((LiricoinParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
            {
                ...
                case MC_PRM_INT32:
                    size=4;
                    if(((LiricoinParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) == MC_PRM_USER)
                    {
                        if(atoll(ptr) > (LiricoinParamArray+i)->m_MaxIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too high: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                        if(atoll(ptr) < (LiricoinParamArray+i)->m_MinIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too low: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                    }
                    if((LiricoinParamArray+i)->m_Type & MC_PRM_DECIMAL)
                    {
                        double d=atof(ptr);
                        if(d >= 0)
                        {
        ...
        {
            if( ((((LiricoinParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_CALCULATED) && 
               (((LiricoinParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_GENERATED)) ||
               (argc > 0) )     
            {
                strcpy(m_lpData+offset,param->m_Name);
                offset+=strlen(param->m_Name)+1;
...
int mc_LiricoinParams::Set(const char *name,const char *source,int source_size)
{
...
int mc_LiricoinParams::Clone(const char* name, mc_LiricoinParams* source)
{
    int err;
...
int mc_LiricoinParams::CalculateHash(unsigned char *hash)
{
    int i;
    int take_it;
    mc_SHA256 *hasher;
...
int mc_LiricoinParams::Validate()
{
    int i,size,offset;
    int isGenerated;
    int isMinimal;
...
int mc_LiricoinParams::Print(FILE* fileHan)
{
    int i,c,size;
    int version;
    int header_printed;
    ...
    fprintf(fileHan,"# ==== LiriCoin configuration file ====\n\n");
    fprintf(fileHan,"# Created by liricoin-util \n");
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"liricoind %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun liricoin-util. \n");
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"liricoind %s\".\n",Name());
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"liricoind %s\".\n",Name());
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence liricoin network configuration. \n");
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"liricoin-util clone %s <new-network-name>\". \n",Name());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running liricoind for this chain. \n");                                    
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"liricoin-util clone %s <new-network-name>\" to generate new network. \n",Name());
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by liricoin-util.\n");
                            fprintf(fileHan,"# They SHOULD ONLY BE EDITED IF YOU KNOW WHAT YOU ARE DOING. \n");
                            break;
                        case MC_PRM_CALCULATED:
                            fprintf(fileHan,"# The following parameters were generated by liricoind.\n");
...
int mc_LiricoinParams::Write(int overwrite)
{
    FILE *fileHan;
    int create;
...
const char* mc_LiricoinParams::Name()
{
    return (char*)GetParam("chainname",NULL);
}

const unsigned char* mc_LiricoinParams::MessageStart()
{
    return (unsigned char*)GetParam("networkmessagestart",NULL);
}

const unsigned char* mc_LiricoinParams::DefaultMessageStart()
{
    return c_DefaultMessageStart;
}


const unsigned char* mc_LiricoinParams::AddressVersion()
{
    return (unsigned char*)GetParam("addresspubkeyhashversion",NULL);
}

const unsigned char* mc_LiricoinParams::AddressScriptVersion()
{
    return (unsigned char*)GetParam("addressscripthashversion",NULL);
}

const unsigned char* mc_LiricoinParams::AddressCheckumValue()
{
    return (unsigned char*)GetParam("addresschecksumvalue",NULL);
}


int mc_LiricoinParams::ProtocolVersion()
{
...
int mc_LiricoinParams::IsProtocolLiricoin()
{
    return m_IsProtocolLiriCoin;
}
...
int mc_Features::VerifySizeOfOpDropElements()
{
    ...
    if(mc_gState->m_NetworkParams->IsProtocolLiricoin() == 0)
    {
        return 1;
    }
