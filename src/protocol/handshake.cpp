...
bool LiricoinNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !LiricoinNode_CanConnect(pnode);
    }
...
bool LiricoinNode_AcceptData(CNode *pnode)
{
    ...
    return !LiricoinNode_DisconnectRemote(pnode);
}
...
bool VerifyLiricoinVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!VerifyLiricoinVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(mc_gState->m_NetworkParams->GetInt64Param("anyonecanconnect") == 0)
    {
        ...
        if(fIsVerackack)
        {
            ...
            if(!LiricoinNode_CanConnect(pfrom))
            {
