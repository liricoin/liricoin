...
bool AcceptLiriCoinTransaction(const CTransaction& tx, 
...
bool ProcessLiricoinVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool PushLiriCoinVerack(CNode* pfrom, bool fIsVerackack);
bool LiricoinNode_CanConnect(CNode *pnode);
bool LiricoinNode_DisconnectRemote(CNode *pnode);
bool LiricoinNode_DisconnectLocal(CNode *pnode);
bool LiricoinNode_RespondToGetData(CNode *pnode);
bool LiricoinNode_SendInv(CNode *pnode);
bool LiricoinNode_AcceptData(CNode *pnode);
bool LiricoinNode_IgnoreIncoming(CNode *pnode);
bool LiricoinNode_IsLocal(CNode *pnode);
...
const string strMessageMagic = "LiriCoin Signed Message:\n";
...
int LiricoinNode_ApplyUpgrades()
{
...
bool LiricoinNode_IsBlockChainSynced(CNode *pnode)
{
...
/*        
        if(mc_gState->m_Features->Streams())
        {
            if(!AcceptLiriCoinTransaction(tx,view,-1,false,reason))
            {
                return state.DoS(0,
                                 error("AcceptToMemoryPool: : AcceptLiriCoinTransaction failed %s : %s", hash.ToString(),reason),
...
/* MCHN START */
        ...
        if(mc_gState->m_Features->Streams())
        {
            if(!AcceptLiriCoinTransaction(tx,view,-1,true,reason, &replay))
            {
                return state.DoS(0,
                                 error("AcceptToMemoryPool: : AcceptLiriCoinTransaction failed %s : %s", hash.ToString(),reason),
                                 REJECT_NONSTANDARD, reason);
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if(mc_gState->m_Features->Streams())
            {
                if(!AcceptLiriCoinTransaction(tx,view,offset,true,reason,NULL))
                {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    ...
                    {
                        return state.DoS(0,
                                         error("ConnectBlock: : AcceptLiriCoinTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                         REJECT_NONSTANDARD, reason);
                    }
                }
                else
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    if(!AcceptLiriCoinTransaction(tx,view,coinbase_offset,true,reason,NULL))
                    {
                        return false;       
                    }
                }
...
/* MCHN START */    
    if(!LiricoinNode_RespondToGetData(pfrom)) 
    {
...
/* MCHN START */
        ...
        {            
            ...
            {
                PushLiriCoinVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolLiricoin())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!ProcessLiricoinVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushLiriCoinVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolLiricoin())
        {
            ...
            if(!ProcessLiricoinVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !PushLiriCoinVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!LiricoinNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !LiricoinNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && LiricoinNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!LiricoinNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && LiricoinNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && LiricoinNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(LiricoinNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
            ...
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && LiricoinNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !LiricoinNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(LiricoinNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=LiricoinNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && LiricoinNode_IsBlockChainSynced(pto))
            {
